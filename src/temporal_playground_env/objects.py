...
import copy
from src.temporal_playground_env.color_generation import sample_color
...
class Thing:
    def __init__(self, object_descr, object_id_int, params):
        ...
        self.object_initial_attributes = dict(
            zip(sorted(self.object_descr.keys()), [[self.object_descr[k]] for k in sorted(self.object_descr.keys())]))
        ...
        # memory of past object information
    ...
    def _sample_color(self):
        ...
        self.initial_rgb_code = self.rgb_code
    ...
    def update_state(self, agent_position, gripper_state, objects, object_grasped, action):
        ...
        # update
        if self.history:
            if self.history[-1].size < self.size:
                self.grow = True
            else:
                self.grow = False
    ...
    def _detect_oscillation(self, previous_values):

        derivative = np.diff(previous_values)
        sign_derivative = np.sign(derivative)

        pair_indices = [i for i in range(len(derivative)) if i % 2 == 0]
        even_indices = [i for i in range(len(derivative)) if i % 2 == 1]
        if 0 not in sign_derivative:
            if np.all(sign_derivative[pair_indices] == sign_derivative[pair_indices][0]) and \
                    np.all(sign_derivative[even_indices] == sign_derivative[even_indices][0]) and \
                    sign_derivative[pair_indices][0] != sign_derivative[even_indices][0] and \
                    np.all(np.abs(derivative) > 0.04 * 0.15):  # NEED TO BE COHERENT WITH BOT SHAKE PATTERN (0.15 is for agent step_size)
                return True
        return False

    def _detect_shake(self):
        shake_time = 6
        if len(self.history) < shake_time:
            return False
        previous_pos = [obj.position for obj in list(self.history)[-shake_time:]]
        previous_x = [pos[0] for pos in previous_pos]
        previous_y = [pos[1] for pos in previous_pos]
        if self._detect_oscillation(previous_x) or self._detect_oscillation(previous_y):
            return True
        else:
            return False
    ...
    def get_features(self):
        ...
        """
        grasped_feature = np.array([1]) if self.grasp else np.array([-1])
...
class Agent():
    ...
    @property
    def position(self):
        return np.array(self.pos, dtype=np.float)

    @property
    def initial_position(self):
        return np.array(self.initial_pos, dtype=np.float)
...
Agent.name = 'you'  # TODO change this
...
obj_type_to_obj = dict(dog=Dog,
                       ...
                       water=Water,
                       thing=Thing,
                       living_thing=LivingThings,
                       animal=Animals,
                       plant=Plants,
                       furniture=Furnitures,
                       supply=Supplies, )

# hacky fix to get names of stuff
for obj_name, obj_class in obj_type_to_obj.items():
    obj_class.name = obj_name.replace('_', ' ')
...
def generate_objects(objects_descr, params):
    ...
    for o in objects_descr:
        assert o[
                   'types'] in obj_type_to_obj.keys(), "The object '{}' is not registered in the obj_type_to_obj dict".format(
            o['types'])

    objs = [obj_type_to_obj[o['types']](o, o_id_int, params) for o, o_id_int in
            zip(objects_descr, range(len(objects_descr)))]
